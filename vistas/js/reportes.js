/*MOSTAR NOMBRE AL CARGAR*/
$(document).ready(function(){
	$('.nombreUsuarioResposteMostrar').text(localStorage.getItem('nombreUsuarioReporte'));
})
...
/*$(document).on('change','.usuarioReporte',function(){
	var usuarioReporte = $($("#servicio option:selected").text()).html();
	console.log("usuarioReporte ",usuarioReporte);
})*/

$(document).on('change', '.usuarioReporte', function(event) {
		var nombre = $(".usuarioReporte option:selected").text();
		if(nombre === 'Seleccionar....'){
			localStorage.setItem('nombreUsuarioReporte', nombre);
			$('.nombreUsuarioResposteMostrar').text(localStorage.getItem('nombreUsuarioReporte'));
		}else{
			localStorage.setItem('nombreUsuarioReporte', nombre);
			$('.nombreUsuarioResposteMostrar').text(localStorage.getItem('nombreUsuarioReporte'));
...
$(document).on('click','.btnCancelarUsuarioReporte',function(){
	 localStorage.removeItem('nombreUsuarioReporte');
	 localStorage.removeItem("capturarRango2");
	 localStorage.clear();
	 window.location = "reportes";
})
